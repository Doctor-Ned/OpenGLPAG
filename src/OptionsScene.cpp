...
OptionsScene::OptionsScene() {
	...
	nextPos.y += 45.0f;
	sliderDifficulty = new UiSlider(SceneManager::getInstance().getUiTextureShader(), BUTTON_IDLE, BUTTON_HOVER,
		BUTTON_CLICKED, nextPos, BUTTON_STANDARD_SIZE
		, SceneManager::getInstance().getUiColorShader(), BUTTON_STANDARD_SIZE.y / 4.0f,
		glm::vec2(BUTTON_STANDARD_SIZE.y * 0.75f, BUTTON_STANDARD_SIZE.y * 0.75f),
		SceneManager::getInstance().difficulty, 0.0f, 4.0f);
	...
	nextPos.y += BUTTON_STANDARD_SIZE.y + 10.0f;
	buttonBackToMenu = new UiTextButton(SceneManager::getInstance().getUiTextureShader(), BUTTON_LONG_IDLE,
