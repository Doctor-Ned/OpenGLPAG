...
PauseScene::PauseScene() {
	...
	background = new UiColorPlane(SceneManager::getInstance().getUiColorShader(), glm::vec4(0.5f, 0.5f, 0.5f, 0.2f), glm::vec2(0.0f, 0.0f), glm::vec2(WINDOW_WIDTH, WINDOW_HEIGHT), false);
	buttonResume = new UiTextButton(SceneManager::getInstance().getUiTextureShader(), BUTTON_LONG_IDLE, BUTTON_LONG_HOVER, BUTTON_LONG_CLICKED, glm::vec2(WINDOW_CENTER_X, WINDOW_HEIGHT / 3.0f)
	...
	buttonMenu = new UiTextButton(SceneManager::getInstance().getUiTextureShader(), BUTTON_LONG_IDLE, BUTTON_LONG_HOVER, BUTTON_LONG_CLICKED, menuPos
		, BUTTON_STANDARD_SIZE, SceneManager::getInstance().getTextRenderer(), "Quit to menu");
